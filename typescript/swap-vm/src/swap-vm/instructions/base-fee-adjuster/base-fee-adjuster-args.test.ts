...
describe('BaseFeeAdjusterArgs', () => {
    ...
    it('should throw on invalid values', () => {
        ...
        expect(
            () => new BaseFeeAdjusterArgs(maxUint64 + 1n, 100n, 100n, 100n)
        ).toThrow()

        expect(() => new BaseFeeAdjusterArgs(100n, -1n, 100n, 100n)).toThrow()
        expect(
            () => new BaseFeeAdjusterArgs(100n, maxUint96 + 1n, 100n, 100n)
        ).toThrow()

        expect(() => new BaseFeeAdjusterArgs(100n, 100n, -1n, 100n)).toThrow()
        expect(
            () => new BaseFeeAdjusterArgs(100n, 100n, maxUint24 + 1n, 100n)
        ).toThrow()

        expect(() => new BaseFeeAdjusterArgs(100n, 100n, 100n, -1n)).toThrow()
        expect(
            () => new BaseFeeAdjusterArgs(100n, 100n, 100n, maxUint64 + 1n)
        ).toThrow()
    })
