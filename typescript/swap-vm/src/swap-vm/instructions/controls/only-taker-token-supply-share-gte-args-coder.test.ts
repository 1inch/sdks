...
describe('OnlyTakerTokenSupplyShareGteArgsCoder', () => {
    ...
    it('should throw on values larger than uint64', () => {
        ...
        expect(
            () => new OnlyTakerTokenSupplyShareGteArgs(TOKEN, tooLarge)
        ).toThrow('Invalid minShareE18 value')
    })

    it('should use static decode method', () => {
        const args = new OnlyTakerTokenSupplyShareGteArgs(
            TOKEN,
            250000000000000000n
        )
        const encoded = coder.encode(args)
        ...
        expect(decoded.token.toString().toLowerCase()).toBe(
