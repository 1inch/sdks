...
describe('OraclePriceAdjusterArgs', () => {
    ...
    it('should encode and decode oracle price adjuster args', () => {
        ...
        const args = new OraclePriceAdjusterArgs(
            maxPriceDecay,
            maxStaleness,
            oracleDecimals,
            oracle
    ...
    it('should throw on invalid values', () => {
        ...
        expect(
            () => new OraclePriceAdjusterArgs(maxUint64 + 1n, 100n, 8n, oracle)
        ).toThrow()
        expect(
            () =>
                new OraclePriceAdjusterArgs(
                    1000000000000000000n,
                    100n,
                    8n,
                    oracle
                )
        ).toThrow('less than 1e18')

        expect(
            () => new OraclePriceAdjusterArgs(100n, -1n, 8n, oracle)
        ).toThrow()
        expect(
            () => new OraclePriceAdjusterArgs(100n, maxUint16 + 1n, 8n, oracle)
        ).toThrow()

        expect(
            () => new OraclePriceAdjusterArgs(100n, 100n, -1n, oracle)
        ).toThrow()
        expect(
            () => new OraclePriceAdjusterArgs(100n, 100n, maxUint8 + 1n, oracle)
        ).toThrow()
    })
